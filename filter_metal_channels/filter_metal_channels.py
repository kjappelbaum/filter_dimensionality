...
from matminer.featurizers.composition import (ElementProperty, OxidationStates, Stoichiometry, ValenceOrbital)
...
from .utils import cross, dot, norm

logger = logging.getLogger()
logger.setLevel(logging.DEBUG)


def get_metallic_substructure(s: Structure,
                              return_non_metal_structure: bool = True,
                              white_list: list = ['O', 'H'],
                              allow_expansion: bool = False,
                              primitive: bool = True,
                              other_metals: list = ['Al', 'In', 'Ga']) -> Structure:
    """
...
def points_in_cylinder(pt1: np.ndarray, pt2: np.ndarray, r: float, q: np.ndarray):
    ...
    return len(np.where(dot(q - pt1, vec) >= 0 and dot(q - pt2, vec) <= 0 \
           and norm(cross(q - pt1, vec)) <= r * norm(vec))[0])
...
def points_in_cylinder_vectorized(pt1: np.ndarray, pt2: np.ndarray, r: float, q: np.ndarray) -> int:
    ...
    return len(np.where((np.einsum('ij, ij->i', q - pt1, vec) >= 0) & (np.einsum('ij, ij->i', q - pt2, vec) <= 0) \
...
def _find_landmarks(metal_radii: np.ndarray, metal_substructure: Structure, metal_coords: np.ndarray,
...
def _find_chain_landmarks(landmarks_z: list, landmarks_y: list, landmarks_x: list, metal_coords: np.ndarray) -> set:
    ...
    for landmark_set in [landmarks_z, landmarks_y, landmarks_x]:
        for i, landmark0 in enumerate(landmark_set):
            for j, landmark1 in enumerate(landmark_set):
...
def _find_good_landmarks(landmarks: list, mc: np.ndarray, metal_substructure: Structure, metal_radii: dict,
                         non_metal_coord: np.ndarray, tolerance_factor_1: float) -> list:
    """
...
def _check_non_metal_in_cylinder(p1: np.ndarray, p2: np.ndarray, metal_substructure: Structure, metal_radii: dict,
                                 non_metal_coord: np.ndarray, tolerance_factor_1: float) -> int:
    """
...
def _get_distance_checked_landmarks(good_landmarks: list,
                                    metal_radii: dict,
                                    metal_substructure: Structure,
                                    tolerance_factor_1: float,
                                    direction_dict: dict = None) -> list:
    """
        ...
        for landmark in good_landmarks:
            ...
            if isinstance(direction, (np.ndarray, list)):
                ...
                if relevant_distance < max_distance:
                    ...
                    logger.debug('the distance was too large between %s for a threshold of %s', landmark, max_distance)
...
def get_channels(metal_substructure: Structure,
    ...
    if mixed_metal:
        ...
        if not (joined_landmarks and landmarks):
            ...
            good_landmarks = _find_good_landmarks(joined_landmarks, metal_coords, metal_substructure, metal_radii,
                                                  non_metal_coord, tolerance_factor_1)
            ...
            good_landmarks = _find_good_landmarks(landmarks, metal_coords, metal_substructure, metal_radii,
                                                  non_metal_coord, tolerance_factor_1)
        ...
        if max_distance_factor and good_landmarks:
            ...
            good_landmarks = _get_distance_checked_landmarks(good_landmarks, metal_radii, metal_substructure,
                                                             max_distance_factor, direction_dict)
        ...
        if good_landmarks:
            ...
            for index, metal in enumerate(metal_coords):
                for i, direction in enumerate(directions):
                    ...
                    pic = _check_non_metal_in_cylinder(p1, p2, metal_substructure, metal_radii, non_metal_coord,
                                                       tolerance_factor_1)
